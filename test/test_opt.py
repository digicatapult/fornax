...
#         ])
#         self.query_result = fornax.opt.QueryResult([tuple(item if item is not None else -1 for item in tup) for tup in self.records])


#     def test_missed(self):
#         _, misses = fornax.opt.missed(self.query_result)
#         self.assertEqual(misses[(3, 13)], 2)
#         self.assertEqual(misses[(2, 5)], 1)

#     def test_totals(self):
#         totals, _ = fornax.opt.missed(self.query_result)
#         self.assertEqual(totals[(3, 13)], 1)
#         self.assertEqual(totals[(2, 5)], 4)

#     def test_get_matching_costs(self):
#         matching_costs = fornax.opt.get_matching_costs(self.query_result)
#         self.assertFalse(True)

#     def test_solve(self):
#         fornax.opt.solve(self.query_result)
